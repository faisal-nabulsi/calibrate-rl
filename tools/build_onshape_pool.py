    ...
    for src in [s.strip() for s in a.sources.split(",") if s.strip()]:
        ...
        for r in ds:
            ...
            if prob is None or ans is None:
                continue
            tag = (str(r.get(sfield)).lower() if sfield else "") + " " + prob[:0]
            if a.amc_only:
                if "amc" not in tag:                         # AMC only
                    continue
                if "aime" in tag and "amc" not in str(r.get(sfield)).lower().replace("aime", ""):
                    continue                                  # drop pure-AIME tagged rows
            m = re.search(r"-?\d+(?:/\d+)?", str(ans).strip())   # numeric (int or simple fraction) answer
            if not m or m.group(0) != str(ans).strip():
                continue                                      # require the WHOLE answer be numeric (clean exact grade)
            prob = prob.strip()
            if prob in amc_probs or prob in seen:
                continue
            seen.add(prob)
            rows.append({"problem": prob, "answer": str(ans).strip(),
